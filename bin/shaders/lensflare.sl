 ...
 *
 * $Revision: 1.2 $    $Date: 2000/11/14 05:53:42 $
 *
 ****************************************************************************/
...
float aspectratio ()
{
    ...
    return (xcomp(Pcorner1)-xcomp(Pcorner0)) / 
...
lensflare ( float intensity = 1.0;
	    ...
	    float rainbowradius = 0.5;
	    float rainbowwidth = 0.2;
	    ...
	    float blotty = 1;
	    float bloony = 1;
	    float spotintensity = 0.08;
	    float spotvarycolor = 0.5;
    ...
    )
{
	...
	float brightness = atten*intensity*(comp(Cl,0)+comp(Cl,1)+comp(Cl,2))/3;
	...
	for (i = 0; i < nspots; i += 1) {
		...
		int = filteredpulse (radius, radius+0.05*axisdist,
				     axisdist, dPndc);
	    } else if (type < (disky+ringy+blotty)) {  /* Soft spot */
